...
def extract_names(filename):
  ...
  pattern_year = re.compile(r"\<h3[^>]*>(.*)(\d{4})</h3>$")
  pattern_name = re.compile(r"<td>(\d+)</td><td>([a-zA-Z]+)</td><td>([a-zA-Z]+)</td>")
  #<tr align="right"><td>3</td><td>Matthew</td><td>Brittany</td>
  list_match = []
  for line in file_name:
    line = line.strip()
    matches_year = pattern_year.findall(line)
    matches_name = pattern_name.findall(line)
    # print(matches)
    for match in matches_year: 
        match_year = list(match)[1]
        print (match_year)
    for match1 in matches_name:
        list_match = list_match.append(match1)
        print (list_match)
